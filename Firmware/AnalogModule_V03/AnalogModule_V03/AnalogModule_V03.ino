...
//            DataFile.close();
    ...
    case 'S': { // Start streaming signal
        StreamSignalToUSB = 1;
    } break;

    case 'X': { // Stop streaming signal
        StreamSignalToUSB = 0;
      } break;
    ...
    case 'R': { // Select ADC Voltage range
        ...
        byte VoltageRangeByte1 = USBCOM.readByte();
        byte VoltageRangeByte2 = USBCOM.readByte(); 
    ...
    case 'A': { // Select active channels
      ...
      case 'P': { // Change sampling frequency
        ...
        //byte samplingPeriod = USBCOM.readByte(); // 
      ...
      case 'G': { // Return the currently loaded parameter file from the microSD card
  ...
  if (StreamSignalToUSB == 1) {
    USBCOM.writeInt16(readOneChannel(ChannelToStream));
  }
...
void readActiveChannels(short unsigned *pdata){
  ...
  for (int i=0; i < nActiveChannels; i++){
    ...
    if (triggered[ActiveChannelsList[i]]==0){
      if (data[i]>ThresholdValue[ActiveChannelsList[i]]){
        ...
        Serial1.write(ActiveChannelsList[i]+1); //CHANGE THIS TO Serial1COM
